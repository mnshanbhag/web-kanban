...
table (FK `ondelete="CASCADE"`) holds each task's subtask checklist items, and a `sprints` table
(`id`, `name`, `start_date`, `end_date`, `status`, `closed_at`) holds sprint records.
